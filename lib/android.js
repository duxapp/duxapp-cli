...
module.exports = {
  ...
  keystore() {
    ...
    const cmd = `${getKeyTools()} -genkeypair -v -storetype PKCS12 -keystore ${global.projectDir}/android/app/${alias}.keystore -alias ${alias} -keyalg RSA -keysize 2048 -validity 1500 -storepass ${password} -dname "CN=(xujian), OU=(jujiang), O=(jujiang), L=(changsha), ST=(hunan), C=(cn)`
  ...
  async packageName(name) {
    ...
    const func1 = data => data.replace(/\"com.duxapp\"/g, `"${name}"`)
    const func2 = data => data.replace(`package ${currentName};`, `package ${name};`)
    const func3 = data => data.replace(`package ${currentName}.wxapi;`, `package ${name}.wxapi;`);
    [
      ['android/app/BUCK', func1],
      ['android/app/build.gradle', func1],
      ['android/app/src/main/AndroidManifest.xml', func1],
      ['android/app/src/main/java/com/duxapp/MainActivity.java', func2],
      ['android/app/src/main/java/com/duxapp/MainApplication.java', func2],
      ['android/app/src/main/java/com/duxapp/wxapi/WXEntryActivity.java', func3],
      ['android/app/src/main/java/com/duxapp/wxapi/WXPayEntryActivity.java', func3]
    ].forEach(item => file.editFile(...item))
    // 更改包名目录结构
    const javaDir = 'android/app/src/main/java/'
    const fromDir = javaDir + currentName.split('.').join('/')

...
module.exports = {
  ...
  async init() {
    ...
    const modules = ['商城', '快速购买', '分销', '文章', 'DIY', '聊天', '钱包', '会员卡', '素材库', '拼券']
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        name: 'module',
        message: '请选择你需要的额外模块',
        default: ['商城', '快速购买', '分销', '文章', 'DIY', '聊天', '钱包'],
        choices: modules,
        pageSize: 10,
        validate(val) {
          if (!val.includes('商城') && val.some(item => ['快速购买', '分销'].includes(item))) {
            return '要选择分销和快速购买模块，必须选中商城'
          }
          return true
        }
      },

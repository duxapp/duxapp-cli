...
module.exports = {
   ...
   * @param {*} local 是否将图标放在本地
   */
  async create(name, app, url, local) {
    ...
    createIconComp(name, fontUrl, obj, local, compPath, woff2)
  }
}

const createIconComp = (name, url, icons, local, compPath, content) => {
...
${local ? `if (process.env.TARO_ENV === 'rn' || process.env.TARO_ENV === 'harmony') {

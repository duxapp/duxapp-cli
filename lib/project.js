...
const project = {
  ...
  update() {
    const copyList = [
      'config', '.editorconfig', '.eslintrc', '.gitignore', 'babel.config.js',
      'global.d.ts', 'jsconfig.json', 'Readme.md'
    ]
    copyList.forEach(fileName => {
      file.copy(path.join(__dirname, '..', 'projectTemplate', fileName), fileName)
    })
    console.log(`更新完成！`)
  },
  /**

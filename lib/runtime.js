...
const _taroConfig = {
  ...
  window: {
    ...
    ..._duxapp.appConfig?.window
  },
  rn: {
    useNativeStack: true,
    ..._duxapp.appConfig?.rn
  }
}
...
`
  ...
  // 创建h5端index.html入口文件
  const createIndexEntry = apps => {
    const last = apps[apps.length - 1]
    const filePath = getPath('src', last, 'index.html')
    let template
    if (existsSync(filePath)) {
...
`
  ...
  const createNpmPackage = apps => {
    editFile('package.json', content => {
      ...
      apps.map(app => {
        const appConfig = readfile('src/' + app + '/app.json')
        const npmConfig = JSON.parse(appConfig).npm || {}
        if (npmConfig.scripts) {
          contents.scripts = { ...contents.scripts, ...npmConfig.scripts }
        }
        if (npmConfig.dependencies) {
          contents.dependencies = { ...contents.dependencies, ...npmConfig.dependencies }
        }
        if (npmConfig.devDependencies) {
          contents.devDependencies = { ...contents.devDependencies, ...npmConfig.devDependencies }
        }
        if (npmConfig.resolutions) {
          contents.resolutions = { ...contents.resolutions, ...npmConfig.resolutions }
        }
      })
  ...
  // 复制兼容文件
  const copyPatches = apps => {
    file.delete('patches')
    apps.forEach(app => {
      const patchesDir = `src/${app}/update/copy/patches`
      if (existsSync(patchesDir)) {
        file.copy(patchesDir, 'patches')
      }
    })
  }

...
`
  ...
  return () => {
    ...
    const configName = getConfigName(customAppsArgv)
...
const editConfig = (() => {
  ...
  return (config, mapping, input, output) => {
    let fileDir
    if (!input) {
      const configName = getConfigName(util.getArgv().find(item => item.startsWith('--app=')))
...
const getConfigName = (appName) => {
  ...
  if (configName && !existsSync(getPath('configs', configName, 'index.js'))) {
    configName = 'default'
  }
  return configName
}

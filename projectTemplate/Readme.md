...
# 调试小程序端 打包基础模块
yarn dev weapp --apps=base
# 调试小程序端 打包多个模块 仅供参考，此命令和yarn dev weapp --apps=shop是一样的，因为shop模块依赖于base模块，当打包shop时会自动打包base，apps的循序会影响页面的顺序，请按需求调用
yarn dev weapp --apps=base,shop
```
...
| ---- index.js  | 模块入口文件，模块初始化的时候将会执行此文件，参考base对应文件 |
| ---- ......  | 其他文件夹表示页面 |
| -- duxapp.js  | 全局配置文件 |
| -- app.js  | 全局入口文件 |
| -- app.scss  | 全局样式 |
...
index.js是每个模块的入口文件，这个文件将会被默认执行，此文件需要默认导出一个对象，如下
...
更新后已经默认不再集成图标库在项目中 如果你需要使用此图标库，需要在入口文件中注册，如下所示
```js
// 可以在此处执行一些要初始化的东西
import { app } from '@/base/utils'
import { defineIcon } from 'taro-design/render'
import icon from 'taro-design/render/static/icon/icon'
import 'taro-design/render/static/icon/icon.css'

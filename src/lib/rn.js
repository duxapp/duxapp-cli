...
export const create = async () => {
  ...
  let configPath = file.pathJoin('configs', configName, 'duxapp.rn.js')
  if (!fs.existsSync(configPath)) {
    configPath = file.pathJoin('configs', configName, 'duxapp.js')
    if (fs.existsSync(configPath)) {
      console.warn(`请将 ${['configs', configName, 'duxapp.js'].join('/')} 文件重命名为 \`duxapp.rn.js\`，在未来的版本中将移除对 duxapp.js 的支持`)
    }
  }
  if (!fs.existsSync(configPath)) {
    console.error(`${configName}/duxapp.rn.js配置文件不存在 请在配置文件夹中创建`)
    process.exit(1)
  }
  ...
  if (!file.existsSync(tempList)) {
    console.log('请安装最新版的 duxappReactNative 模块后重试')
    process.exit(1)
  }
  ...
  if (process.platform === 'darwin') {
    await util.asyncSpawn('chmod +x ./gradlew', {
  ...
  const configs = (await Promise.all(apps.map(async app => {
    ...
    let configPath = file.pathJoin('src', app, 'update', 'index.js')
    if (fs.existsSync(configPath)) {
      console.warn(`请将 ${['src', app, 'update', 'index.js'].join('/')} 文件重命名为 \`rn.js\`，在未来的版本中将移除对 index.js 的支持`)
    }
    let rnjs = file.pathJoin('src', app, 'update', 'rn.js')
    if (fs.existsSync(rnjs)) {
      configPath = rnjs
    }
    // 复制模块文件
    let copyPath = file.pathJoin('src', app, 'update', 'copy')
    if (fs.existsSync(copyPath)) {
      const copyPathList = file.dirAndFileList(copyPath)
      if (copyPathList.length > 1 || (copyPathList.length && !copyPathList.includes('patches')))
        console.warn(
          `请将 ${['src', app, 'update', 'copy'].join('/')
          } 文件夹内除了 \`patches\` 以外的所有文件移动到同级目录的 \`copy.rn\` 文件夹内`
        )
    }
    let rncopy = file.pathJoin('src', app, 'update', 'copy.rn')
    if (fs.existsSync(rncopy)) {
      copyPath = rncopy
    }
    if (fs.existsSync(copyPath)) {
      file.copy(copyPath, '')
  ...
  // 指定要编译哪些平台
  if (duxappConfig.android.reactNativeArchitectures) {
    fileExec.replace('android/gradle.properties', 'reactNativeArchitectures', duxappConfig.android.reactNativeArchitectures)
  }
  // hermes引擎
  ...
  // 指定gradle下载地址
  if (duxappConfig.android.distributionUrl) {
    fileExec.replace('android/gradle/wrapper/gradle-wrapper.properties', 'distributionUrl', duxappConfig.android.distributionUrl)
  }
  ...
  // 复制用户文件
  let userCopyPath = file.pathJoin(`configs/${configName}/copy`)
  if (fs.existsSync(userCopyPath)) {
    console.warn(`请将 ${`configs/${configName}/copy`} 文件夹重命名为 copy.rn，在未来的版本中将移除对 copy 的支持`)
  }
  let userrncopy = file.pathJoin(`configs/${configName}/copy.rn`)
  if (fs.existsSync(userrncopy)) {
    userCopyPath = userrncopy
  }
  if (fs.existsSync(userCopyPath)) {
    file.copy(userCopyPath, '')
  }
...
/**
 * 生成app图标，将logo原图放在项目配置文件夹下，使用这个命令生成ios和安卓的图标
 * @function
 * @param filePath [可选]logo路径，默认: logo.png
 */
export const logo = async (filePath = 'logo.png') => {
  const configName = await util.getConfigName()
  ...
  console.log('正在通过 icon.wuruihong.com 生成启动图标')
  // 上传图片
  ...
  zip.on('ready', () => {
    const copyPath = path.join('configs', configName, 'copy.rn')
